...
class CategoryController extends Controller
{
    ...
    function showRecursive()
    {
        ...
        if (isset($_GET['m']))
            ...
            return view("showCategories", ["table" => html_entity_decode($this->table),"message" => $this->getMessage()]);
    ...
    //go through tree of categories iteartively (Depth first search)
    function showIteratively()
    {
        $stack = new Stack(); //create a stack
        $visited = array();   //array to keep track of what categories were visited.
        $categories = \App\Category::all()->where('parent',0); //get all categories that has no parrents
        $depth = 0;
        foreach ($categories as $category)
        {
            $category->depth = 0;
            $stack->push($category);    //push all these parrents to the stack.
        }
        while(!$stack->isEmpty()) //go through if there is any elements still needed to be visited
        {
            ...
            foreach ($categories as $category) //go through children
            {
...
public function addNewCategory(Request $request)
{
   ...
   if (strlen($name) > 0) //check if name entered
   {
       ...
       if ($parent != 0) //if element has parrent use its indent - if not indent is zero
